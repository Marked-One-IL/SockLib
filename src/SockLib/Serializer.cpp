...
{
    this->serializeBytes(reinterpret_cast<const SockLib::Helper::Byte*>(&i), static_cast<SockLib::Helper::Size>(sizeof(i)));
}
...
void SockLib::Serializer::serializeBytes(const SockLib::Helper::Byte* bytes, SockLib::Helper::Size size)
{
    const SockLib::Helper::Size oldSize = static_cast<SockLib::Helper::Size>(this->m_bytes.size());
    this->m_bytes.resize(oldSize + size, SockLib::Helper::Byte{});
    std::memcpy(&this->m_bytes[oldSize], bytes, static_cast<std::size_t>(size));
}
...
void SockLib::Serializer::serializeStr(std::string_view v)
{
    ...
    this->serializeBytes(reinterpret_cast<const SockLib::Helper::Byte*>(v.data()), static_cast<SockLib::Helper::Size>(v.size()));
}

const SockLib::Helper::Byte *SockLib::Serializer::getBytes(void) const
{
    return this->m_bytes.data();
}
SockLib::Helper::Size SockLib::Serializer::getSize(void) const
{
    return static_cast<SockLib::Helper::Size>(this->m_bytes.size());
}